...
const islamicCategories = ["islam", "sufi", "tasawuf", "aqidah", "akhlak", "fiqih"];
...
async function run() {
  console.log("Starting batched generation of 50 ISLAMIC articles with semantic dedup...");
  let count = 0;
  ...
  while (count < 50) {
    ...
    while (batchCount < 5 && count < 50) {
      const cat = islamicCategories[count % islamicCategories.length];
      console.log(`Generating ISLAMIC article ${count + 1}/50 for category: ${cat}`);

      try {
        const idea = await generateJSON(`
          Generate 1 unique and profound "Mental Nutrition" article idea for category: ${cat}.
          Strictly unique topic, deeply rooted in Islamic wisdom.
          Avoid: dhikr, tazkiyat, neuroplasticity, cognitive hygiene, cognitive gatekeeping — these are overused.
        ...
        const article = await generateText(`Write a "Mental Nutrition" article in Bahasa Indonesia based on Islamic wisdom. Title: ${idea.title}. Category: ${cat}. Angle: ${idea.angle}. Length: 750-900 words. Use Markdown. Reflective question at the end.`, { temperature: .7 });
    ...
    if (batchQueries.length > 0) {
      const tempFile = path.join(process.cwd(), `tmp_batch_islamic_${Date.now()}.sql`);
      await fs.writeFile(tempFile, batchQueries.join("\n"), 'utf8');

      console.log(`Executing batch of ${batchQueries.length} ISLAMIC articles on D1 remote...`);
      try {
  ...
  console.log("Done: 50 ISLAMIC articles with semantic dedup.");
}
